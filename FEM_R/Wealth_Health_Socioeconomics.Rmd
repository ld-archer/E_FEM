...
```{r}
## REMEMBER ACCOUNTING! SMOKING VARS AND EXSTAT

# which variables to select and impute
# Need to do this in parts because some of the lags wouldn't impute when 
# the current vars were included. Specifically smoken, smokev, and problem_drinker
demog.vars <- c('idauniq', 'wealth_quint', 'age', 'male')
risk.vars.fir <- c('smokev', 'smoken', 'heavy_smoker', 'drink', 
                   'problem_drinker', 'logbmi',  'exstat', 'l2drink', 
                   'l2logbmi', 'l2exstat')
risk.vars.sec <- c('heavy_smoker', 'drink', 'logbmi',  'exstat', 'l2smoken',
                   'l2smokev', 'l2heavy_smoker', 'l2drink', 'l2problem_drinker',
                   'l2logbmi', 'l2exstat')

## Have a go at multiple imputation using distribution from median quintile first
# First generate a test dataset with only the variables we need to impute and age,sex
test.fir <- repl %>% filter(year == 2012, wealth_quint %in% c(1, 3)) %>%
                  select(all_of(demog.vars), all_of(risk.vars.fir))

test.sec <- repl %>% filter(year == 2012, wealth_quint %in% c(1, 3)) %>%
                  select(all_of(demog.vars), all_of(risk.vars.sec))

# Remove data for the risk behaviours for wealth_quint == 1
# Doing this in chunks as not sure of a better way
#test.1 <- test[test$wealth_quint == 1]
#test.3 <- test[test$wealth_quint == 3]

test.1.fir <- filter(test.fir, wealth_quint == 1)
test.1.sec <- filter(test.sec, wealth_quint == 1)
test.3.fir <- filter(test.fir, wealth_quint == 3)
test.3.sec <- filter(test.sec, wealth_quint == 3)

# replace all risk var information with NA for lowest quintile
test.1.fir[risk.vars.fir] <- NA
test.1.sec[risk.vars.sec] <- NA

# Now put them all back together with a rbind
test.fir <- rbind(test.1.fir, test.3.fir)
test.sec <- rbind(test.1.sec, test.3.sec)


# Now have a go at imputing all missings
imp <- mice(data=test.fir, m=5, maxit=10, printFlag = FALSE)

# get a complete version of the dataset
test2.fir <- complete(imp)

imp2 <- mice(data=test.sec, m=5, maxit=10, printFlag = FALSE)

test2.sec <- complete(imp2)

test2 <- merge(test2.fir, test2.sec, by = 'idauniq')

rm(test.1.fir, test.1.sec, test.3.fir, test.3.sec, risk.vars.fir, risk.vars.sec,
   test.fir, test.sec)
```
...
if (is.na(test2$l2smoken)) {
  test2$l2smoken <- test2$smoken
}

# if l2smokev == 1 then smokev must also == 1
if (is.na(test2$smokev)) {
  test2$smokev <- test2$l2smokev
...
# so assume same here
if (is.na(test2$l2problem_drinker)) {
  test2$l2problem_drinker <- test2$problem_drinker
}
...
```{r}
## Step 2 is now to replace the values from the original repl dataset and save this as an alternative for running
# through the model
...
write_dta(repl.final, 'input_data/ELSA_repl_modified_quint1.dta')
```
