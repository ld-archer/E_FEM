...
compareSurvivalCurve <- function(cohort, intervention, int_name, save=FALSE,
  ...
  p1 <-ggplot(data = survivals, aes(x=Age, y=Life_Exp, color = Source)) +
        geom_smooth(lwd = 0.5) +
...
### Survival Curve
```{r}
c <- read_dta('output/SCENARIO/ELSA_core_cohort/ELSA_core_cohort_by_rep.dta')
s <- read_dta('output/SCENARIO/ELSA_core_remove_smoken/ELSA_core_remove_smoken_by_rep.dta')
...
compareSurvivalCurveConfints <- function(cohort, intervention, int_name, save=FALSE,
  ...
  p <-ggplot(data = survivals, aes(x=Age, y=Survival, color = variable)) +
        geom_line() +
        labs(x = 'Age', y = 'Survival')
...
compareSurvivalCurveConfints(c_rep, s_rep)
```
