...
rm(lnly2, sociso2, lnly.check2, sociso.check2)
```
...
rm(form.mort.min, probit.mort.min)
```
...
rm(form.mort.ls, probit.mort.ls)
```
...
rm(form.mort.ls2, probit.mort.ls2)
```
...
rm(form.mort.full, probit.mort.full)
```
...
rm(form.mort.test, probit.mort.test)
```

AIC: 6668.7

```{r}
form.mort.test2 <- 'died ~ male + white + hsless + college + 
                  l2age65l + l2age6574 + l2age75p + 
                  l2lnly1 + l2lnly2 +
                  l2sociso1 + l2sociso2 +
                  l2cancre + l2diabe + l2hearte + l2lunge +
                  l2stroke + l2smokev + l2smoken +
                  l2alcfreq7 + l2alcfreq8 +
                  l2srh4 + l2srh5'

probit.mort.test2 <- glm(formula = form.mort.test2, family = binomial(link = 'probit'),
                    data = trans2)

summary(probit.mort.test2)

rm(form.mort.test2, probit.mort.test2)
```
...
rm(form.lnly.min, probit.lnly.min, form.lnly, probit.lnly, trans.lnly)
```
...
```{r}
## TEST
form.lnly.test <- 'lnly ~ male + hsless + college + scale(l2age65l) + scale(l2age6574) + scale(l2age75p) + l2cohab + l2widowed + l2single + l2employed + l2inactive + l2physact + l2anyadl + l2anyiadl + l2srh5 + l2socyr + l2gcareinhh1w + scale(l2logatotb) + l2cesd'
...
rm(form.lnly.test, probit.lnly.test)
```
...
# Get a sample comparable to the sample used in FEM
trans.sociso <- trans %>%
  dplyr::select(cwtresp, idauniq, wave, died, sociso, male, white, hsless, college, l2age65l, l2age6574, l2age75p, atotb, l2employed, l2inactive, l2physact) %>%
  filter(wave > 1) %>%
  filter(died == 0)
...
rm(trans.sociso, trans.sociso.complete, form.sociso.min, probit.sociso.min, form.sociso, probit.sociso)
```
...
rm(form.sociso.test, probit.sociso.test)
```
...
```{r}
trans.notDead$adlstat <- as.factor(trans.notDead$adlstat)
trans.notDead$iadlstat <- as.factor(trans.notDead$iadlstat)
## MINIMAL
...
rm(form.adlstat.min, probit.adlstat.min, form.adlstat, probit.adlstat)
```
...
rm(form.adlstat.test, probit.adlstat.test)
```
...
rm(form.iadlstat.min, probit.iadlstat.min, form.iadlstat, probit.iadlstat)
```
...
rm(form.iadlstat.test, probit.iadlstat.test)
```
...
```{r}
# ## MINIMAL
# form.tr20.min <- 'tr20 ~ male + scale(l2age65l) + scale(l2age6574) + scale(l2age75p) | male + scale(l2age65l) + scale(l2age6574) + scale(l2age75p)'
...
# 
# ## TEST
# form.tr20 <- 'tr20 ~ male + white + hsless + college + scale(l2age65l) + scale(l2age6574) + scale(l2age75p) | male + white + hsless + college + scale(l2age65l) + scale(l2age6574) + scale(l2age75p)'
...
# summary(probit.tr20)
# 
# rm(form.tr20.min, probit.tr20.min, form.tr20, probit.tr20)
```
...
# probit.tr20.test <- zeroinfl(formula = form.tr20.test,
#                         data = trans.notDead,
#                         na.action = na.omit,
#                         weights = cwtresp)
# print('Full model:')
...
# 
# rm(form.tr20.test, probit.tr20.test)
```
...
rm(form.sight.min, probit.sight.min, form.sight, probit.sight)
```
...
rm(form.hearing.min, probit.hearing.min, form.hearing, probit.hearing)
```
...
```{r}
trans.notDead$srh <- as.factor(trans.notDead$srh)

## MINIMAL
form.srh.min <- 'srh ~ male + scale(l2age65l) + scale(l2age6574) + scale(l2age75p)'
probit.srh.min <- polr(formula = form.srh.min,
                    data = trans.notDead,
                    Hess = TRUE,
                    na.action = na.omit,
                    weights = cwtresp,
                    method = 'probit')
print('Minimal model:')
summary(probit.srh.min)

## TEST
form.srh <- 'srh ~ male + white + hsless + college + scale(l2age65l) + scale(l2age6574) + scale(l2age75p) + l2logbmi_l30 + l2logbmi_30p + l2smoken + l2smokev + l2hearte + l2diabe + l2lunge + l2stroke + l2adl1 + l2adl2 + l2adl3p + l2iadl1 + l2iadl2p + l2employed + l2inactive + l2sight + l2hearing + l2physact'
probit.srh <- polr(formula = form.srh,
                    data = trans.notDead,
                    Hess = TRUE,
                    na.action = na.omit,
                    weights = cwtresp,
...
#$dvars $lvars_age l2logbmi_l30 l2logbmi_30p `lvars_smoke' `lvars_workstat' `lvars_funclimit' l2hearte l2diabe l2lunge l2stroke

rm(form.srh.min, probit.srh.min, form.srh, probit.srh)
```

AIC: 107499.08 
AIC: 78886.70 : Sight, hearing, and physact


# Depression

```{r}
trans.notDead$cesd <- as.factor(trans.notDead$cesd)

## MINIMAL
form.cesd.min <- 'cesd ~ male + scale(l2age65l) + scale(l2age6574) + scale(l2age75p)'
probit.cesd.min <- polr(formula = form.cesd.min,
                    data = trans.notDead,
                    Hess = TRUE,
                    na.action = na.omit,
                    weights = cwtresp,
                    method = 'probit')
print('Minimal model:')
summary(probit.cesd.min)

## TEST
form.cesd <- 'cesd ~ male + white + hsless + college + scale(l2age65l) + scale(l2age6574) + scale(l2age75p) + l2logbmi_l30 + l2logbmi_30p + l2smoken + l2smokev + l2stroke + l2cancre + l2diabe + l2hearte + l2gcareinhh1w + l2lnly + l2sociso + l2physact + l2anyadl + l2anyiadl + l2alcfreq7 + l2alcfreq8 + l2srh4 + l2srh5'
probit.cesd <- polr(formula = form.cesd,
                    data = trans.notDead,
                    Hess = TRUE,
                    na.action = na.omit,
                    weights = cwtresp,
