...
ly.all <- ly.all %>% select(scenario, everything())

#rm(ly.coh, ly.lnly, ly.sociso, ly.lnly.sociso, ly.list)
```
...
ly.change <- ly.all %>%
  ...
  select(outcome, contains('L_'), contains('S_'), contains('LS_'), -contains('No_Social_Isolation')) %>%
...
# arrange columns properly
ly.combined <- ly.combined %>% select(scenario, everything())
...
```{r}
lnly.check <- repl %>%
  select(hhidpn, year, age, male, lnly) %>%
  filter(year == 2012) %>%
  group_by(lnly, male) %>%
  count()
...
lnly.check5 <- repl %>%
  filter(year == 2012) %>%
  select(hhidpn, year, age, male, childless, lnly) %>%
