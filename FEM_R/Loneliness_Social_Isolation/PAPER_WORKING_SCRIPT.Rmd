...
counts <- s %>% summarise_each(funs(sum(!is.na(.)))) %>%
  dplyr::select(age, male, bmi, smoken, smokev, hsless, college) %>%
  t() %>%
  as.data.frame() %>%
...
rm(design, means, mean.df, counts, s)
```
...
[1] "No Loneliness"
[1] "Cohort Average Disability Free Lifeyears:         12.55"
[1] "Intervention Average Disability Free Lifeyears:   13.67"
[1] "Increase:                                         1.121"
[1] "No Social Isolation"
[1] "Cohort Average Disability Free Lifeyears:         12.55"
[1] "Intervention Average Disability Free Lifeyears:   12.63"
[1] "Increase:                                         0.08628"
[1] "No Loneliness OR Social Isolation"
[1] "Cohort Average Disability Free Lifeyears:         12.55"
[1] "Intervention Average Disability Free Lifeyears:   13.76"
[1] "Increase:                                         1.208"
...
```
[1] "No Loneliness"
[1] "Cohort Average Disease Free Lifeyears:         5.48"
[1] "Intervention Average Disease Free Lifeyears:   5.668"
[1] "Increase:                                         0.1873"
[1] "No Social Isolation"
[1] "Cohort Average Disease Free Lifeyears:         5.48"
[1] "Intervention Average Disease Free Lifeyears:   5.623"
[1] "Increase:                                         0.1427"
[1] "No Loneliness OR Social Isolation"
[1] "Cohort Average Disease Free Lifeyears:         5.48"
[1] "Intervention Average Disease Free Lifeyears:   5.852"
[1] "Increase:                                         0.3715"

#### Visualise

Lets write some functions to visualise this information, so will need to collect them into dataframes first.

```{r}

ly.lnly <- lifeyearComparison(cohort_full, no.lonely, 'L')
ly.sociso <- lifeyearComparison(cohort_full, no.sociso, 'S')
ly.lnly.sociso <- lifeyearComparison(cohort_full, no.lnly.sociso, 'LS')

ly <- rbind(ly.lnly, ly.sociso)
ly.all <- rbind(ly.lnly, ly.sociso, ly.lnly.sociso)
ly.wide <- ly %>% 
  pivot_wider(names_from = scenario,
                              values_from = lifeyear,
                              names_repair = 'unique')
colnames(ly.wide) <- c('outcome', 'scenario', 'cohort', 'intervention')

disab.lnly <- disabilityFreeLifeYearsComp(cohort_full, no.lonely, 'L')
disab.sociso <- disabilityFreeLifeYearsComp(cohort_full, no.sociso, 'S')
disab.lnly.sociso <- disabilityFreeLifeYearsComp(cohort_full, no.lnly.sociso, 'LS')

disab <- rbind(disab.lnly, disab.sociso)
disab.all <- rbind(disab.lnly, disab.sociso, disab.lnly.sociso)
disab.wide <- disab %>% 
  pivot_wider(names_from = scenario,
                              values_from = lifeyear,
                              names_repair = 'unique')
colnames(disab.wide) <- c('outcome', 'scenario', 'cohort', 'intervention')

disease.lnly <- diseaseFreeLifeYearsComp(cohort_full, no.lonely, 'L')
disease.sociso <- diseaseFreeLifeYearsComp(cohort_full, no.sociso, 'S')
disease.lnly.sociso <- diseaseFreeLifeYearsComp(cohort_full, no.lnly.sociso, 'LS')

disease <- rbind(disease.lnly, disease.sociso)
disease.all <- rbind(disease.lnly, disease.sociso, disease.lnly.sociso)
disease.wide <- disease %>% 
  pivot_wider(names_from = scenario,
                              values_from = lifeyear,
                              names_repair = 'unique')
colnames(disease.wide) <- c('outcome', 'scenario', 'cohort', 'intervention')

lifeyears <- rbind(ly.wide, disab.wide, disease.wide)
lifeyears['change'] <- lifeyears['intervention'] - lifeyears['cohort']

rm(ly, ly.all, ly.wide, ly.lnly, ly.sociso, ly.lnly.sociso)
rm(disab.lnly, disab.sociso, disab.lnly.sociso, disab, disab.all, disab.wide)
rm(disease.lnly, disease.sociso, disease.lnly.sociso, disease, disease.all, disease.wide)
```

Now plot.

```{r}
ggplot(data = filter(lifeyears, outcome == 'LifeYears'),
       aes(x = scenario, y = change, color = scenario, fill = scenario)) +
  geom_col()

ggplot(data = filter(lifeyears, outcome == 'DisabilityFree'),
       aes(x = scenario, y = change, color = scenario, fill = scenario)) +
  geom_col()

ggplot(data = filter(lifeyears, outcome == 'DiseaseFree'),
       aes(x = scenario, y = change, color = scenario, fill = scenario)) +
  geom_col()

combined <- ggplot(data = lifeyears, aes(x = factor(scenario, levels = c('L', 'S', 'LS')), y = change, fill = scenario)) +
  geom_col() +
  facet_wrap(~factor(outcome, levels = c('LifeYears', 'DisabilityFree', 'DiseaseFree'))) +
  labs(x = 'Scenario', y = 'Increase') +
  scale_fill_discrete(labels = c('No Loneliness', 'No Social Isolation'))
  #scale_color_manual(labels = c('No Loneliness', 'No Loneliness or Social Isolation', 'No Social Isolation', values = c('red', 'green', 'blue')))

combined

# Save plot into FEM_R/Loneliness_Social_Isolation/plots/
path <- 'FEM_R/Loneliness_Social_Isolation/plots/'

ggsave(filename = 'ly_dfly_facet_L_S.png',
      plot = combined,
      path = path,
      width = 9,
      height = 6)
...
Lets start by appending detailed output and working on a function to get it into a plottable format.

```{r}
cohort <- detailed_output_append(base.out.dir = 'output/LNLY_SOCISO_TEST/',
...
coh.test <- ly_outcomes_from_detailed_output(cohort)


ggplot(data = coh.test, aes(x = outcome, y = mean, fill = outcome)) +
...
First combine detailed outputs for each of the interventions we want to investigate.
...
rm(ly.lnly, ly.sociso, ly.lnly.sociso, ly.list)
rm(nolnly, nosociso, both)
```

Now we have a dataframe with the LY outcomes (and confidence intervals) for all scenarios. We need to reformat the dataset and plot these values with the confidence intervals clearly shown.
...
ly.change$scenario <- factor(ly.change$scenario, levels = c('L', 'S', 'LS'))
...
ggsave(filename = 'ly_dfly_facet_all_confints.png',
      plot = combined.plot,
      path = path,
      width = 9,
      height = 6)

rm(ly.all, ly.change, combined.plot)
```

### Realistic Intervention

```{r}
lnly.real <- detailed_output_append(base.out.dir = out.dir,
                              scenario = 'ELSA_real_lnly')
lnly.sociso <- detailed_output_append(base.out.dir = out.dir,
                              scenario = 'ELSA_real_sociso')
```

```{r}
ly.lnly.real <- ly_outcomes_from_detailed_output(lnly.real)
ly.sociso.real <- ly_outcomes_from_detailed_output(lnly.sociso)

ly.lnly.real$scenario <- 'Loneliness'
ly.sociso.real$scenario <- 'Social_Isolation'

ly.comb.list <- list(ly.coh, ly.lnly.real, ly.sociso.real)
ly.combined <- do.call(rbind, ly.comb.list)

# arrange columns properly
ly.combined <- ly.combined %>% 
  dplyr::select(scenario, everything())

ly.combined$scenario <- factor(ly.combined$scenario, levels = c('Baseline', 'Loneliness', 'Social_Isolation'))
ly.combined$outcome <- factor(ly.combined$outcome, levels = c('LY', 'DiseaseFLY', 'DisabilityFLY'))
```
...
```{r}
# Now do change plots starting with the ly.all object
ly.change.real <- ly.combined %>%
  pivot_wider(names_from = 'scenario',
              values_from = c('mean', 'min', 'max', 'margin')) %>%
  mutate(L_change = mean_Loneliness - mean_Baseline,
         S_change = mean_Social_Isolation - mean_Baseline,
         L_min = L_change - margin_Loneliness,
         L_max = L_change + margin_Loneliness,
         S_min = S_change - margin_Social_Isolation,
         S_max = S_change + margin_Social_Isolation) %>%
  dplyr::select(outcome, contains('L_'), contains('S_')) %>%
...
ly.change.real$scenario <- factor(ly.change.real$scenario, levels = c('L', 'S', 'LS'))
  
combined.plot <- ggplot(data = ly.change.real, aes(x = scenario, y = change, fill=scenario)) +
  geom_col() +
  facet_wrap(~outcome) +
  geom_errorbar(aes(ymin = min, ymax = max)) +
  labs(title = 'Life Years, Disease Free Life Years, and Disability Free Life Years') +
...
Need to get the detailed output for this, but this will be a really nice thing to include.
We can show the improvements in similar plots by different subgroups. Hopefully we'll see bigger improvements in specific subgroups, perhaps a certain age group or gender. The literature I was reading about this suggests that each intervention should affect both genders differently, so should be interesting. 

#### Data Checks

As a first step, lets get the breakdown by gender and age group of loneliness and social isolation (for planning interventions).

```{r}
lnly.check <- repl %>%
  dplyr::select(hhidpn, year, age, male, lnly) %>%
  filter(year == 2012) %>%
  group_by(lnly, male) %>%
  count()
...
lnly.check5 <- repl %>%
  filter(year == 2012) %>%
  dplyr::select(hhidpn, year, age, male, childless, lnly) %>%
  group_by(lnly, male, childless) %>%
  count()
```
...
```{r}
coh.male <- cohort %>% filter(male == 1)
coh.female <- cohort %>% filter(male == 0)
ly.male <- ly_outcomes_from_detailed_output(coh.male)
ly.female <- ly_outcomes_from_detailed_output(coh.female)

ly.male$gender <- 'Male'
ly.female$gender <- 'Female'

coh.gender <- rbind(ly.male, ly.female)
coh.gender <- coh.gender %>% 
  dplyr::select(gender, outcome, everything())
...
ly.disease <- coh.gender %>%
  filter(outcome == 'DiseaseFLY') %>%
  dplyr::select(-outcome)

ly.disability <- coh.gender %>%
  filter(outcome == 'DisabilityFLY') %>%
  dplyr::select(-outcome)

# Now do change plots starting with the ly.all object
ly.change.gender <- coh.gender %>%
  ...
  #       S_min = S_change - margin_Social_Isolation,
  #       S_max = S_change + margin_Social_Isolation) %>%
  #select(outcome, contains('L_'), contains('S_')) %>%
  #pivot_longer(cols = L_change:S_max,
  #             names_sep = '_',
  #             names_to = c('scenario', 'statistic'),
  #             values_to = 'LifeYears') %>%
...
```{r}
test <- detailed_output_append(base.out.dir = 'output/COMPLETE/',
                              scenario = 'ELSA_core_cohort')
```
