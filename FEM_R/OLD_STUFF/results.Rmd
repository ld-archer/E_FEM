...
visCDComparisonCohort <- function(cohort, int, int_name, cd, save=FALSE, path='/home/luke/Documents/E_FEM_clean/Deliverables/E-FEM_Paper/figures/results/') {
  ...
  selcohort <- cohort %>%
              dplyr::select(c('Age', all_of(var))) %>%
              dplyr::rename('Cohort' = var)
  
  selInt <- int %>%
              dplyr::select(c('Age', all_of(var))) %>%
              dplyr::rename('Intervention' = var)
  
  melted <- merge(selcohort, selInt, by = 'year') %>%
              reshape2::melt(id.var = 'Age', value.name = 'Scenario')

  p <- ggplot(data = melted, aes(x = Age, y = Scenario, color = variable)) +
          geom_smooth(lwd = 0.5) +
...
visCDComparisonCohort(c, s, 'No Smoke', 'cancre')
visCDComparisonCohort(c, s, 'No Smoke', 'diabe')
visCDComparisonCohort(c, s, 'No Smoke', 'hearte')
visCDComparisonCohort(c, s, 'No Smoke', 'lunge')
visCDComparisonCohort(c, s, 'No Smoke', 'stroke')
visCDComparisonCohort(c, s, 'No Smoke', 'smoken')
visCDComparisonCohort(c, s, 'No Smoke', 'employed')
visCDComparisonCohort(c, s, 'No Smoke', 'retired')
visCDComparisonCohort(c, s, 'No Smoke', 'anyadl')
visCDComparisonCohort(c, s, 'No Smoke', 'anyiadl')
...
visList <- visCDComparisonBMI(c, bmi1, bmi2, bmi3, cds, save=TRUE)
...
visCDComparison(c, bmi1, 'No Obesity', 'diabe', save=TRUE)
