...
Second go at this cross-validation, attempting to solve some issues by using the 60 plus population.
...
vis_CV2_prev <- function(long, CV2) {
  ...
  # Start with cancre, diabe, hearte
  cd_l2 <- select(l2, c(year, age, cancre, diabe, hearte, lunge, smoken, bmi, employed, anyadl)) #, anyiadl, adl1, adl2, adl3p))
  cd_CV2 <- select(CV2, c(year, a_age_60p, p_cancre_60p, p_diabe_60p, 
                            p_hearte_60p, p_lunge_60p, p_smoken_60p, 
                            a_bmi_60p, p_employed_60p,  p_anyadl_60p))
                            #, p_anyiadl_60p p_adl1_60p, p_adl2_60p, p_adl3p_60p))
  
  CV2_rename <- cd_CV2 %>% dplyr::rename('age' = a_age_60p,
                                        'cancre' = p_cancre_60p, 
                                        'diabe' = p_diabe_60p, 
                                        'hearte' = p_hearte_60p,
                                        'lunge' = p_lunge_60p,
                                        'smoken' = p_smoken_60p,
                                        'bmi' = a_bmi_60p,
                                        'employed' = p_employed_60p,
                                        'anyadl' = p_anyadl_60p)
                                        #'anyiadl' = p_anyiadl_60p)
                                        #'adl1' = p_adl1_60p,
                                        #'adl2' = p_adl2_60p,
                                        #'adl3p' = p_adl3p_60p)
...
diseases <- c('hearte', 'hibpe', 'stroke', 'diabe', 'lunge', 'demene', 'asthmae')
...
ageUK <- filter(ageUK, Condition != 'Cancer, recent')
