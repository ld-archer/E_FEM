...
# Rename age column
m_copy <- dplyr::rename(m_copy, age_elsa = "rownames(m_copy)")
f_copy <- dplyr::rename(f_copy, age_elsa = "rownames(f_copy)")
...
## Now check against US mortality adjustment
...
male <- m_cohort %>% 
            dplyr::rename(age = "Attained age (years)") %>%
            filter(age >= 50)
...
rownames(f_copy) <- 1:nrow(f_copy)
# Rename age column
m_copy <- dplyr::rename(m_copy, age = "rownames(m_copy)")
f_copy <- dplyr::rename(f_copy, age = "rownames(f_copy)")
