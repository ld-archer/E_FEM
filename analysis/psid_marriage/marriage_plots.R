...
sname = c("psid")
sdesc = c("PSID Default")

print("Loading sources:")
for(s in sname) { 
	# load each scenario's summary output
	fname = paste("../../output/",s,"/",s,"_summary.Rdata",sep="")
	print(fname)
	load(file=fname) 
	ls()
	
	# drop records before year.start
	eval(parse(text=paste(s,"_summary=",s,"_summary[",s,"_summary$year > 2009,]",sep="")))
}
ls()
...
### Median age
#plotlines("psid", "", xvar="year", yvar="med_age", "Year", "Age (years)", ymult=1.0, 40, 60, "Median Age")

### Incidence of marriage, cohab, single, widowed, partdied
yvars = c("imarried","icohab","isingle","iwidowed","ipartdied")
ydescs = c("Marriage","Cohabitation","Single (not partner death)","Widowed","Partner death")
plotline2("psid_summary", "year", yvars, ydescs, "Year", "Incidence", 1.0, 0.0, 0.13, "Incidence of Marital Status")
#append.ggplots.8x8pdf(pl1, pl.age +  ylab("Median Age"), pdfname="imarried_icohab_isingle_age.pdf")

### Transitions from single
yvars = c("prob_single_single","prob_single_cohab","prob_single_married")
ydescs = c("Single","Cohabitation","Marriage")
plotline2("psid_summary", "year", yvars, ydescs, "Year", "Incidence", 1.0, 0.0, 1.0, "Transitions from Single")

### Transitions from cohab
yvars = c("prob_cohab_single","prob_cohab_cohab","prob_cohab_married","prob_cohab_partdied")
ydescs = c("Single (not partner death)","Cohabitation","Marriage","Partner death")
plotline2("psid_summary", "year", yvars, ydescs, "Year", "Incidence", 1.0, 0.0, 1.0, "Transitions from Cohabitation")

### Transitions from married
yvars = c("prob_married_single","prob_married_cohab","prob_married_married","prob_married_widowed")
ydescs = c("Single (not widowed)","Cohabitation","Marriage","Widowed")
plotline2("psid_summary", "year", yvars, ydescs, "Year", "Incidence", 1.0, 0.0, 1.0, "Transitions from Married")


### Incidence of marriage and first marriage
yvars = c("imarried","ifmarried")
ydescs = c("Any Marriage","First Marriage")
plotline2("psid_summary", "year", yvars, ydescs, "Year", "Incidence", 1.0, 0.0, 0.25, "Incidence of Marriage")
