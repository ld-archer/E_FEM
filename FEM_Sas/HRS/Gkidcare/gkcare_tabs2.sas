...
%do w=&bw %to &ew;
   ...
   proc means data=tmp (where=(inw&w=1)
                        rename=(r&w.anygkcare=anygkcare
                                ...
                                r&w.agey_e=age
                                nkids%substr(&year,3,2)=nkids
                                aigkcare%substr(&year,3,2)=gkcarehrs_ifany));
      class nkids ragender race raeduc age;
      types () nkids nkids*ragender nkids*race nkids*raeduc nkids*age;
   ...
   data stats&year;
      ...
      length byv $ 10;
      year=&year;
      if max(ragender,race,raeduc,age,nkids)=. then byv="All";
...
run;
proc sort data=tabs;
 by byv year anykids ragender race raeduc age;

ods html file="gcare_tabs2_yr.xls" style=minimal;
proc print data=tabs noobs label;
   id byv;
   id anykids ragender race raeduc age;
   var year
