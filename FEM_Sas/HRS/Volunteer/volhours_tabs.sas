...
Do some tables on volunteer hours last 2 years.
...
data tmp;
   merge imp.volhours (in=_in1)
    ...
    array volhours_[*] %wvlist(r,volhours,begwv=4);
    array anyvol_[*] %wvlist(r,anyvol,begwv=4);
    array volcond_[*] %wvlist(r,volcond,begwv=4);
    
    do i=1 to dim(volhours_);
       if volhours_[i] ne . then do;
          anyvol_[i]=(volhours_[i]>0);
          if volhours_[i]>0 then volcond_[i]=volhours_[i];
       end;
    end;
    race=raracem;
...
%do w=&bw %to &ew;
   ...
   proc means data=tmp (where=(inw&w=1)
                        rename=(r&w.anyvol=anyvol
                                r&w.volhours=volhrs
                                r&w.agey_e=age
                                r&w.volcond=volhrs_ifany));
      class ragender race raeduc age;
      types () ragender race raeduc age;
      var anyvol volhrs volhrs_ifany;
      weight r&w.wtresp;
      format age agecat. race racer.;
...
ods html file="volhours_tabs_yr.xls" style=minimal;
