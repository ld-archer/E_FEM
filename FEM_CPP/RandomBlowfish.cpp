...
// The specification of the blowfish algorithm initializes the S and P
// boxes mentioned above to the hexadecimal digits of PI. 
void RandomBlowfish::set_pi()
{
  int j;
  // The hexadecimal digits of pi, arranged as four s_boxes and one p_array, as per the Blowfish default.
  // From : http://www.schneier.com/code/constants.txt
...
// End of hexidecimal pi digits

  // This copies the data from the arrays into the S and P boxes. We don't
  // want to just initialize at compile time because if we are going to
  // rekey, we need to reset the S boxes and P boxes back to the digits
  // of pi and then compute the new subkeys */
  for (j = 0; j < 256; j++) {
    s[0][j] = sbox0[j];
    s[1][j] = sbox1[j];
    s[2][j] = sbox2[j];
...
// Description: The non-linear function used in the Feistel network below.
//
// Arguments:
//   x: A 32-bit word.
unsigned RandomBlowfish::Feistel( word_T x ) const
{
  return ((s[0][x.p.byte0] + s[1][x.p.byte1]) ^
    s[2][x.p.byte2]) + s[3][x.p.byte3];
}

// Description: Encrypts two halves of a number.
//
// Remarks: Blowfish is a Feistel network, meaning that the plaintext is split
// into two halves, and then one half is operated on by an F function and then
// used to update the other half. For the next round, the blocks are switched.
// This structure garantees invertibility of the whole thing, even if F is
// non-linear and not invertible. For purposes of generating random numbers,
// we don't care if the algorithm is invertible, so this part could be
// tweaked a little bit without caring about funking up the Feistel network.
//
// Arguments:
//   block_left: Left block.
//   block_right: Right block.
void RandomBlowfish::encrypt( unsigned *block_left, unsigned *block_right ) const
{
  register word_T xL, xR;
  ...
  *block_left = xR.w ^ p[17];
  *block_right = xL.w ^ p[16];
}

// Description: Takes a key for the algorithm as an array of bytes, and
// computes the necessary information for the algorithm to run with this key.
//
// Arguments:
//   key: The key for the algorithm as an array of bytes.
void RandomBlowfish::keySchedule( unsigned char *key )
{
  int i,j;
  unsigned xL, xR;
...
RandomBlowfish::RandomBlowfish(unsigned int sd, unsigned int r) : Random(r) {
	alreadySetup = false;
	setSeed(sd);
	setup();
}

unsigned int RandomBlowfish::randomIndex(unsigned int id, Random_t rand) const {
	return Vars::NVars*rand+id;
}

// Description: Sets blowfish_seed to a value provided 
//
// Remarks: This value will supercede the MODEL_MONTECARLO_KEY
// value as the random number seed.
//
// Arguments:
//   seed: New blowfish seed.
void RandomBlowfish::setSeed(unsigned int newSeed )
{
  seed = newSeed;
}

// Description: Takes a 32-bit "seed" value and generates a blowfish
// key and then calls the key schedule for the algorithm.
//
// Remarks: This 32-bit value is set by compile time constant
// MONTECARLO_KEY. By using the fixed seed and not passing it in as an argument, it makes it impossible for
// the random number generator to change state suddenly because somebody called
// the setup function again with a different value. Calling the setup
// function in the middle of the run with the same seed will have
// absolutely no effect on the random numbers generated. It does not
// reset the generator, it just sets up the sbox and pbox subkey
// arrays for the algorithm, which are deterministic and thus the same
// if the same key is used.
//
// All random values generated by the algorithm are repeatable if the
// key (or 32-bit seed here) is the same as well as the setup used to
// generate the plaintext for the algorithm. 
//
// We prepare a blowfish key (18 32-bit words (448 bits)) from the 32-bit
// "MONTECARLO_KEY" by some silly operations to create a rich mix of 1 
// and 0 bits. This is not strictly necessary, but leaving the rest of 
// the 18-word key as zero values just didn't give me a warm fuzzy 
// feeling about the dependence of the generator on the seed value we
// use as a key.
void RandomBlowfish::setup()
{
  unsigned key[18];

  unsigned seed1 = seed;
  unsigned seed2 = ~((seed >> 16) | (seed << 16));

  for (int i = 0; i < 18; i += 2) {
    key[i] = seed1; 
    key[i+1] = seed2;
    seed1 = (seed1 << 7) | (seed2 & 0x7F);
    seed2 = (seed2 >> 7) | (seed1 & 0xFE000000);
  }
...
// Description: Uses blowfish encryption to generate a 32-bit random number
// from a 32-bit input and two 16 bit inputs.
//
// Remarks: Note that blowfish is a 64-bit block cipher, so we first create a
// 64-bit plaintext. The resulting 64-bit ciphertext is converted to a 32-bit
// value by spanking the left and right 32-bit halves together. We rotate the
// right word by 16 bits before XORing because the Feistel network above uses
// XOR to mix the words during its operation.  XORing without rotating might
// reduce the avalance property of the final round.
//
// Returns: The 32-bit random number.
//
// Arguments:
//   id32: The 32bit input.
//   id16_1:  The first 16 bit input
//   id16_2:  The second 16 bit input.
unsigned RandomBlowfish::getRand( unsigned id32, unsigned id16_1, unsigned id16_2 ) const
{
  if (!alreadySetup) {
    throw fem_exception("RandomBlowfish needs to be set up before first use");
  }

  unsigned left = ((id16_1 & 0xFFFF) << 16) | (id16_2 & 0xFFFF);
  unsigned right = id32;

  encrypt(&left, &right);
  return (left ^ ((right >> 16) | (right << 16)));
}

double RandomBlowfish::uniformDist( unsigned int id,  unsigned int process, unsigned int year ) const
{
  return getRand(id, process, year)/(double) UINT_MAX;
}


double RandomBlowfish::normalDist( unsigned int id,  unsigned int process, unsigned int year, double m, double sigma ) const
{
	return normal_dist(uniformDist(id, process, year), m, sigma);
}
