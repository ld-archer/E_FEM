...
global ster    "$fem_path/Estimates/Partial"
global outdata "$fem_path/Input_yh"
global netdir  "/homer/c/Retire/FEM/rdata"
global outdir "$fem_path/Input_yh/partial"
...
*** Rename the vectors for iearnx and hatotax,as well as rename variables
	do "$workdir/init_transition_rename_iearnx.do"
...
foreach n in iearnx{
	...
	save "$indata\\`n'_simu.dta", replace	
	restore
}
...
do "$workdir/init_transition_rename_hatotax.do"
...
save "$indata\\hatotax_simu.dta", replace	
...
do "$workdir/put_est.mata"
 foreach var in  hatotax iearnx {
 		...
 		capture erase "$outdir//vm`var'"
 		capture erase "$outdir//vs`var'"
		mata: _putestimates("$outdir//vm`var'","$outdir//vs`var'" ,"v`var'")
 }
  /*
clear mata
...
cap log close
cd "\\zeno\zeno_a\DOL\Input_yh\partial"
* cd "\\zeno\zeno_a\DOL\Input_yh\all"
	#d ;
 	estout b_*
 	 	using "newhazards_yh.txt", 
...
foreach d in $order {
	...
	quietly{
		log using "$outdir//hatota_92_04.txt", replace
		local _counter = 1
	...
	quietly{
		log using "$outdir//iearnx.txt", replace
		local _counter = 1
