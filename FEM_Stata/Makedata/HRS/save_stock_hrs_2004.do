...
Construct the stock_hrs_2004 file, the restricted 2004 cohort file
...
local expand : env EXPAND
  ...
  use $dua_rand_hrs/all2004r_pop_adjusted.dta
...
if(floor(c(version))>=14) {
	saveold "$dua_rand_hrs/stock_hrs_2004.dta",replace v(12)
}
else{
	saveold "$dua_rand_hrs/stock_hrs_2004.dta",replace
}
...
est save $resmodels/fraime_nonzero2004.ster, replace
regress fraime `covars' if fraime_nonzero
est save $resmodels/fraime2004.ster, replace
reg flogq `covars' [aw=weight]
est save $resmodels/flogq2004.ster, replace
reg rpia `covars' [aw=weight]
est save $resmodels/rpia2004.ster, replace
