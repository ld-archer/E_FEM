...
label define hwtfinl_lbl 0000000000 `"0000000000"'
label values hwtfinl hwtfinl_lbl
...
tab hsless college, m r col cell

tempfile basefile
save `basefile', replace

collapse (mean) hsless college, by(year age)
...
use `basefile', clear
collapse (mean) hsless college [aweight=wtfinl], by(year age)
gen phsless_04_tmp=hsless if year==2004
egen phsless_04=min(phsless_04_tmp)
gen phsless_weighted=hsless/phsless_04

gen pcollege_04_tmp=college if year==2004
egen pcollege_04=min(pcollege_04_tmp)
gen pcollege_weighted=college/pcollege_04

drop *_04_tmp *_04
rename college college_weighted
rename hsless hsless_weighted

tempfile wght
save `wght', replace

merge 1:1 year using `unwght'
drop _merge

lowess hsless year, mean generate(hsless_smooth)
lowess college year, mean generate(college_smooth)
