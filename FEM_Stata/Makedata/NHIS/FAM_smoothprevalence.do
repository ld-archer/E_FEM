...
local agelow = 18
use "$outdata/FAM_nhis97plus_selected.dta"
...
* Collapse data 
keep if inrange(age, 18,52)
* gen sex = male + 1

collapse $poutcome [aw = wtfa_sa], by(year age)
label drop _all

keep if inrange(age,18,53)
sort year age, stable

save "$outdata/FAM_pred.dta", replace
